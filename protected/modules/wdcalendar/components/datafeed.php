...
function addCalendar($identr,$st, $et, $sub, $ade){
  $ret = array();
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "insert into `jqcalendar` (`id_entr`, `subject`, `starttime`, `endtime`, `isalldayevent`) values ('"
      .mysql_real_escape_string($identr)."', '"
...
function addDetailedCalendar($identr,$st, $et, $sub, $ade, $dscr, $loc, $color, $tz){
  $ret = array();
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "insert into `jqcalendar` (`id_entr`, `subject`, `starttime`, `endtime`, `isalldayevent`, `description`, `location`, `color`) values ('"
      .mysql_real_escape_string($identr)."', '"
      .mysql_real_escape_string($sub)."', '"
      .php2MySqlTime(js2PhpTime($st))."', '"
      .php2MySqlTime(js2PhpTime($et))."', '"
...
function listCalendarByRange($sd, $ed){
  ...
  try{
    ...
    $sql = "select * from `jqcalendar` where (`starttime` between '"
      .php2MySqlTime($sd)."' and '". php2MySqlTime($ed)."') AND (`id_entr`='" .$compte->id_entr."')";
...
switch ($method) {
    case "add":
        $ret = addCalendar(Yii::app()->user->id,$_POST["CalendarStartTime"], $_POST["CalendarEndTime"], $_POST["CalendarTitle"], $_POST["IsAllDayEvent"]);
        break;
    case "list":
    ...
    case "adddetails":
        ...
        if(isset($_GET["id"]) && (int)$_GET['id']){
            ...
            $ret = addDetailedCalendar(Yii::app()->user->id,$st, $et,                    
                $_POST["Subject"], isset($_POST["IsAllDayEvent"])?1:0, $_POST["Description"], 
                $_POST["Location"], $_POST["colorvalue"], $_POST["timezone"]);
        }        
        break; 
